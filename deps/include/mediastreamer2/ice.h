...
typedef struct _stunLongTermAuthToken
{
	char *realm;
	char *username;
	char *passwd;
	char *nonce;
} stunLongTermAuthToken;
/**
...
typedef struct _IceStunServerCheck {
	...
	IceStunServerCheckTransaction transanction;
...
/**
 * Add long term token for ICE session's STUN/TURN server
 * Should be used with ice_session_gather_candidates() 
 */
MS2_PUBLIC void ice_session_add_long_term_token(IceSession *session, stunLongTermAuthToken *token);
