...
typedef struct zkrecorder_cb_data {
	void *userdata;
	void (*on_data)(unsigned char *data, int len, void *userdata);
} zkrecorder_cb_data;
...
struct _AudioStream
{
	...
	MSFilter *recorder;
	MSFilter *zk_recorder_tee;
	MSFilter *zk_recorder;
	char *recorder_file;
...
MS2_PUBLIC int audio_stream_start_full_record(AudioStream *stream, RtpProfile *profile, const char *rem_rtp_ip,int rem_rtp_port,
	const char *rem_rtcp_ip, int rem_rtcp_port, int payload,int jitt_comp, const char *infile, const char *outfile,
