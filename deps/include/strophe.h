 ...
 */
#define XMPP_NS_STANZA "urn:ietf:params:xml:ns:xmpp-stanzas"
/** @def XMPP_NS_PING
...
/** @def XMPP_NS_X
		http://xmpp.org/extensions/xep-0077.html
 */
#define XMPP_NS_X "jabber:x:data"

#define XMPP_NS_MUC "http://jabber.org/protocol/muc"

#define  XMPP_NS_MUC_USER "http://jabber.org/protocol/muc#user"

#define XMPP_NS_MUC_OWNER "http://jabber.org/protocol/muc#owner"

#define  XMPP_NS_CONFERECE "jabber:x:conference"

#define XMPP_NS_SOURCE "zonekey:x:source"
...
typedef enum {
	...
	XMPP_SE_REG_ERROR,					// 
	XMPP_SE_REG_UNSUPPORT,				// 不支持 in-band registration
	XMPP_SE_REG_USERNAME_CONFLICT,		// 406
	XMPP_SE_REG_CANCEL,					// 409
} xmpp_error_type_t;
...
typedef void (*xmpp_reg_handler)(xmpp_conn_t * const conn,
				const int error,
				void * const userdata);
...
//void xmpp_conn_free(xmpp_conn_t * const conn);
...
//先身份认证, 认证失败时进行in-band注册;
int xmpp_connect_client_ex(xmpp_conn_t * const conn, 
			const char * const altdomain,
			unsigned short altport,
			xmpp_conn_handler callback,
			void * const userdata);
...
/** 支持 in-band registration
 */
int xmpp_reg_register(xmpp_conn_t * const conn, const char *jid,
			const char *passwd, const char *email,
			xmpp_reg_handler callback, void *const userdata);
