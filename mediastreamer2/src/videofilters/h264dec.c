...
static void dec_process(MSFilter *f){
	...
	while((im=ms_queue_get(f->inputs[0]))!=NULL){
		...
		if (!ms_queue_empty(&nalus)){
			...
			if (f->outputs[1]!=NULL )
			{
				mblk_t *h264_frame = allocb(2*size, 0);
				mblk_set_timestamp_info(h264_frame, d->unpacker.last_ts);
				memcpy(h264_frame->b_wptr, d->bitstream, size);
				h264_frame->b_wptr +=size;
				ms_queue_put(f->outputs[1],h264_frame);
			}
