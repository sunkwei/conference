...
static void audio_stream_free(AudioStream *stream) {
	...
	if (stream->zk_recorder) ms_filter_destroy(stream->zk_recorder);
	if (stream->zk_recorder_tee) ms_filter_destroy(stream->zk_recorder_tee);
	...
	MSSndCard *playcard, MSSndCard *captcard, bool_t use_ec)
{
	...
	// ½ûÓÃ jitter buffer
	rtp_session_enable_adaptive_jitter_compensation(rtps, 0);
	rtp_session_enable_jitter_buffer(rtps, 0);
...
int audio_stream_start_full_record(AudioStream *stream, RtpProfile *profile, const char *rem_rtp_ip,int rem_rtp_port,
	const char *rem_rtcp_ip, int rem_rtcp_port, int payload,int jitt_comp, const char *infile, const char *outfile,
	MSSndCard *playcard, MSSndCard *captcard, bool_t use_ec,  zkrecorder_cb_data *data)
{
	RtpSession *rtps=stream->ms.session;
	PayloadType *pt,*tel_ev;
	int tmp;
	...
	stream->zk_recorder = ms_filter_new_from_desc(&zkrecorder_desc);
	ms_filter_call_method(stream->zk_recorder, ZKRECORDER_METHOD_SET_CALLBACK, data);
	stream->zk_recorder_tee = ms_filter_new(MS_TEE_ID);
	ms_filter_call_method(stream->zk_recorder_tee,MS_TEE_UNMUTE,&record_pin);

	ms_connection_helper_start(&h);
	ms_connection_helper_link(&h,stream->ms.rtprecv,-1,0);
	ms_connection_helper_link(&h,stream->ms.decoder,0,0);
	ms_connection_helper_link(&h,stream->zk_recorder_tee, 0, 0);
	...
	ms_filter_link(stream->zk_recorder_tee, 1, stream->zk_recorder, 0);
	/*call recording part, attached to both outgoing and incoming graphs*/
	if (stream->recorder){
		ms_filter_link(stream->send_tee,1,stream->recorder_mixer,0);
		ms_filter_link(stream->recv_tee,1,stream->recorder_mixer,1);
...
AudioStream *audio_stream_new(int loc_rtp_port, int loc_rtcp_port, bool_t ipv6){
	...
	stream->zk_recorder = 0;
	stream->zk_recorder_tee = 0;
	return stream;
}
...
void audio_stream_stop(AudioStream * stream)
{
	if (stream->ms.ticker){
		...
		if (stream->dummy){
			...
			if (stream->zk_recorder_tee)
				ms_connection_helper_unlink(&h, stream->zk_recorder_tee, 0, 0);
			...
			if (stream->zk_recorder_tee && stream->zk_recorder)
				ms_filter_unlink(stream->zk_recorder_tee, 1, stream->zk_recorder, 0);
