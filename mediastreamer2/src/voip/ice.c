...
static void ice_send_turn_server_allocate_request(ortp_socket_t sock, IceSession *session, IceStunServerCheck *check);
...
static void ice_check_list_gather_candidates(IceCheckList *cl, Session_Index *si)
{
	...
	if ((cl->rtp_session != NULL) && (cl->gathering_candidates == FALSE) 
		&& (cl->state != ICL_Completed) && (ice_check_list_candidates_gathered(cl) == FALSE)) {
		...
		if (sock > 0) {
			...
			if (si->index == 0) {
				...
				//turnSendAllocate(sock, &turn_server, NULL, NULL, NULL);
				ice_send_turn_server_allocate_request(sock, cl->session, check);
...
void ice_session_add_long_term_token(IceSession *session, stunLongTermAuthToken *token)
{
	session->auth_token = ms_list_append(session->auth_token, token);
}
...
static void ice_send_turn_server_allocate_request(ortp_socket_t sock, IceSession *session, IceStunServerCheck *check)
{
	//const struct sockaddr_in *servaddr = (struct sockaddr_in *)&session->ss;
	// StunAddress4 turn_server;
	...
	//turn_server.addr = htonl(servaddr->sin_addr.s_addr);
	//turn_server.port = htons(servaddr->sin_port);
	memset(&msg, 0, sizeof(StunMessage));
	memset(&username,0,sizeof(username));
	memset(&password,0,sizeof(password));
	// FIXME：sunkw 暂时先不要支持 TURN 了
#if 0
	stunBuildRequest(&msg, NULL);
#endif //
...
static void ice_send_stun_server_checks(IceStunServerCheck *check, IceCheckList *cl)
{
	...
	if (ice_compare_time(curtime, check->next_transmission_time) >= 0) {
		int n = ms_list_size(check->transactions);
		if (n < ICE_MAX_STUN_REQUEST_RETRANSMISSIONS) {
			check->next_transmission_time = ice_add_ms(curtime, ICE_DEFAULT_RTO_DURATION * pow(2, n));
