...
typedef struct _TeeData{
	...
	int mode;	// ZONEKEY: 当 mode 为 0 时，缺省属性，msg 仅仅引用计数加一，如果 1，则复制完整 msg 到每个 pin
}TeeData;

static void tee_init(MSFilter *f){
	f->data=ms_new0(TeeData,1);
	((TeeData*)f->data)->mode = 0;
}
...
static int tee_set_mode(MSFilter *f, void *arg)
{
	TeeData *d = (TeeData*)f->data;
	d->mode = *(int*)arg;
	return 0;
